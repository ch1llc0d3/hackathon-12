...
test('normalizeControl never emits the typed value', () => {
  const field = normalizeControl({
    ref: 'vt-0', tag: 'input', type: 'text', id: 'nif',
    forLabel: 'Nº de identificación fiscal', value: '12345678Z', required: true,
  });
  assert.equal(field.filled, true);
  assert.equal('value' in field, false);
  assert.equal(JSON.stringify(field).includes('12345678Z'), false);
});
...
test('normalizeControl cleans and caps select options', () => {
  const field = normalizeControl({
    ...
    options: ['  Álava ', 'Albacete', '', ...Array.from({ length: 40 }, (_, i) => `P${i}`)],
  });
  assert.equal(field.options[0], 'Álava');
